...
import net.menthor.abstractionRecognition.wizard.RefactoringPage;

public class NonSortalAbstractionRefactoringPage extends RefactoringPage {

	public NonSortalAbstractionOccurrence asscyc;
	 ...
	 * Create the wizard.
	 */
	public NonSortalAbstractionRefactoringPage(NonSortalAbstractionOccurrence asscyc) {
		super();
		this.asscyc = asscyc;

		setTitle(NonSortalAbstraction.getAbstractionInfo().acronym + " Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public NonSortalAbstractionWizard getAssCycWizard() {
		return (NonSortalAbstractionWizard) getWizard();
	}
	...
	public IWizardPage getNextPage() {
		((NonSortalAbstractionWizard) getWizard()).removeAllActions();
		...
		if (btnDerive.getSelection() && assocCombo.getSelectionIndex() >= 0) {
			...
			// Action =============================
			NonSortalAbstractionAction newAction = new NonSortalAbstractionAction(asscyc);
		...
		return ((NonSortalAbstractionWizard) getWizard()).getFinishing();
	}
}
