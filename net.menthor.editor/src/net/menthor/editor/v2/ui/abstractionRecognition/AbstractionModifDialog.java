 ...
 */
public class AbstractionModifDialog extends Dialog {
	 ...
	 */
	public AbstractionModifDialog(Shell parentShell) {
		super(parentShell);
	}

	public AbstractionModifDialog(Shell parentShell, Fix fix, JFrame frame) {
		super(parentShell);
		this.fix = fix;
		this.frame = frame;
	}
	...
	public static int openDialog(final Fix fix, final JFrame frame) {
		...
		AbstractionModifDialog resultDialog = new AbstractionModifDialog(shell, fix, frame);
		resultDialog.create();
	...
	public void addFix() {
		for (Object obj : fix.getModified())
			modifiedList.add(OntoUMLNameHelper.getCompleteName((EObject) obj));
		for (Object obj : fix.getAdded())
			addedList.add(OntoUMLNameHelper.getCompleteName((EObject) obj));
		for (Object obj : fix.getDeleted())
			removedList.add(OntoUMLNameHelper.getCompleteName((EObject) obj));
	...
	protected Control createDialogArea(Composite parent) {
		...
		introLabel.setText("Congratulations! You successfully verified the pattern.");
		...
		gl_container.setVerticalGroup(gl_container.createParallelGroup(GroupLayout.LEADING)
				.add(gl_container.createSequentialGroup().add(10).add(introLabel).add(18).add(modifiedLabel).add(8)
						.add(modifiedList, GroupLayout.DEFAULT_SIZE, 93, Short.MAX_VALUE).add(6).add(addedLabel).add(6)
						.add(addedList, GroupLayout.DEFAULT_SIZE, 93, Short.MAX_VALUE).add(10).add(removedLabel).add(6)
						.add(removedList, GroupLayout.DEFAULT_SIZE, 93, Short.MAX_VALUE).add(11).add(rulesLabel).add(6)
						.add(rulesText, GroupLayout.DEFAULT_SIZE, 93, Short.MAX_VALUE).add(43).add(statusLabel)
						.add(21)));
	...
	public void create() {
		...
		getShell().setText("Pattern Modifications");
		addFix();
	}
