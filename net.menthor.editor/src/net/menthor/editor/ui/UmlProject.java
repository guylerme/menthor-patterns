...
/**
 * The UmlProject is serialized to a binary file in order to store the diagrams
 * and its graphics allElements.
 */
public class UmlProject implements Serializable {
	...
	public AntiPatternList getAntipatterns() {
		return antipatterns;
	}

	public void setAntipatterns(AntiPatternList antipatterns) {
	...
	private transient AbstractionList abstractions;

	public AbstractionList getAbstractions() {
		return abstractions;
	}

	public void setAbstractions(AbstractionList abstractions) {
		this.abstractions = abstractions;
	}

	private transient List<OclDocument> oclDocList = new ArrayList<OclDocument>();

	public List<OclDocument> getOclDocList() {
		return oclDocList;
	}

	public void setOclDocList(List<OclDocument> oclDocList) {
		this.oclDocList = oclDocList;
	}

	public void clearOclDocs() {
		this.getOclDocList().clear();
	}
	...
	/**
	 * The model (contained in a resource) is serialized separately, by its own
	 * means so here it is transient.
	 */
	...
	public OntoUMLParser getRefParser() {
		if (refparser == null && getModel() != null)
			refparser = new OntoUMLParser(getModel());
		return refparser;
	}
	...
	public void clearOpenedDiagrams() {
		if (openedDiagrams != null)
			openedDiagrams.clear();
		else
			openedDiagrams = new ArrayList<Integer>();
	}
	...
	public void saveAsOpened(OntoumlDiagram diagram) {
		...
		if (index >= 0) {
			if (openedDiagrams != null) {
				if (!openedDiagrams.contains(index))
					openedDiagrams.add(index);
			}
		}
	}
	...
	public boolean isAllClosed() {
		...
		if (openedDiagrams != null) {
			for (OntoumlDiagram diagram : diagrams)
				if (openedDiagrams.contains(diagram))
					allClosed = false;
		}
		return allClosed;
	}
	...
	public Set<Generalization> getGeneralizations() {
		...
		for (PackageableElement item : getElements()) {
			if (item instanceof RefOntoUML.Class) {
				generalizations.addAll(((RefOntoUML.Class) item).getGeneralization());
			}
		}
	...
	public String getTempDir() {
		if (tempDir == null)
			tempDir = DirectoryUtil.makeTempDir();
		return tempDir;
	}

	public String getBinDir() {
		if (binDir == null)
			binDir = DirectoryUtil.makeBinDir();
		return binDir;
	}
