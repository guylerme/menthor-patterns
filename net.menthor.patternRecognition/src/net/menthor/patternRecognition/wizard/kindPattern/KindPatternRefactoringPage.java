...
public class KindPatternRefactoringPage extends RefactoringPage {
	
	public KindOccurrence asscyc;
	 ...
	 * Create the wizard.
	 */
	public KindPatternRefactoringPage(KindOccurrence asscyc) 
	{
		super();	
		this.asscyc = asscyc;
		
		setTitle(KindPattern.getPatternInfo().acronym+" Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public KindPatternWizard getAssCycWizard(){
		return (KindPatternWizard)getWizard();
	}
	...
	{
		((KindPatternWizard)getWizard()).removeAllActions();
		...
		if (btnDerive.getSelection() && assocCombo.getSelectionIndex()>=0)
		{
			...
			//Action =============================
			KindPatternAction newAction = new KindPatternAction(asscyc);
		...
		return ((KindPatternWizard)getWizard()).getFinishing();
	}
}
