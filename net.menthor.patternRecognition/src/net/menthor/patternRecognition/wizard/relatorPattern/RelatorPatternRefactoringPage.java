...
public class RelatorPatternRefactoringPage extends RefactoringPage {
	
	public RelatorOccurrence asscyc;
	 ...
	 * Create the wizard.
	 */
	public RelatorPatternRefactoringPage(RelatorOccurrence asscyc) 
	{
		super();	
		this.asscyc = asscyc;
		
		setTitle(RelatorPattern.getPatternInfo().acronym+" Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public RelatorPatternWizard getAssCycWizard(){
		return (RelatorPatternWizard)getWizard();
	}
	...
	{
		((RelatorPatternWizard)getWizard()).removeAllActions();
		...
		if (btnDerive.getSelection() && assocCombo.getSelectionIndex()>=0)
		{
			...
			//Action =============================
			RelatorPatternAction newAction = new RelatorPatternAction(asscyc);
		...
		return ((RelatorPatternWizard)getWizard()).getFinishing();
	}
}
