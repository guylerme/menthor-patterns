...
public class ModePatternRefactoringPage extends RefactoringPage {
	
	public ModeOccurrence asscyc;
	 ...
	 * Create the wizard.
	 */
	public ModePatternRefactoringPage(ModeOccurrence asscyc) 
	{
		super();	
		this.asscyc = asscyc;
		
		setTitle(ModePattern.getPatternInfo().acronym+" Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public ModePatternWizard getAssCycWizard(){
		return (ModePatternWizard)getWizard();
	}
	...
	{
		((ModePatternWizard)getWizard()).removeAllActions();
		...
		if (btnDerive.getSelection() && assocCombo.getSelectionIndex()>=0)
		{
			...
			//Action =============================
			ModePatternAction newAction = new ModePatternAction(asscyc);
		...
		return ((ModePatternWizard)getWizard()).getFinishing();
	}
}
