...
public class SubstanceSortalPatternRefactoringPage extends RefactoringPage {

	public SubstanceSortalOccurrence substanceSortal;
	 ...
	 * Create the wizard.
	 */
	public SubstanceSortalPatternRefactoringPage(SubstanceSortalOccurrence substanceSortal) {
		super();
		this.substanceSortal = substanceSortal;

		setTitle(SubstanceSortalPattern.getPatternInfo().acronym + " Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public SubstanceSortalPatternWizard getAssCycWizard() {
		return (SubstanceSortalPatternWizard) getWizard();
	}
	...
	public void createControl(Composite parent) {
		...
		for (Association a : substanceSortal.getOnlyAssociations()) {
	...
	public IWizardPage getNextPage() {
		((SubstanceSortalPatternWizard) getWizard()).removeAllActions();

		if (btnDerive.getSelection() && assocCombo.getSelectionIndex() >= 0) {
			Association assoc = (Association) substanceSortal.getOnlyAssociations().get(assocCombo.getSelectionIndex());
			// Action =============================
			SubstanceSortalPatternAction newAction = new SubstanceSortalPatternAction(substanceSortal);
		...
		return ((SubstanceSortalPatternWizard) getWizard()).getFinishing();
	}
}
