...
public class PhasePatternRefactoringPage extends RefactoringPage {

	public PhaseOccurrence pOccur;
	 ...
	 * Create the wizard.
	 */
	public PhasePatternRefactoringPage(PhaseOccurrence pOccur) {
		super();
		this.pOccur = pOccur;

		setTitle(PhasePattern.getPatternInfo().acronym + " Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public PhasePatternWizard getAssCycWizard() {
		return (PhasePatternWizard) getWizard();
	}
	...
	public IWizardPage getNextPage() {
		((PhasePatternWizard) getWizard()).removeAllActions();
		...
		if (btnDerive.getSelection() && assocCombo.getSelectionIndex() >= 0) {
			...
			// Action =============================
			PhasePatternAction newAction = new PhasePatternAction(pOccur);
		...
		return ((PhasePatternWizard) getWizard()).getFinishing();
	}
}
