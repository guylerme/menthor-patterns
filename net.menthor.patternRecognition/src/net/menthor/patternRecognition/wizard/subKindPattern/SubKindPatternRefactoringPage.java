...
public class SubKindPatternRefactoringPage extends RefactoringPage {

	public SubKindOccurrence skOccur;
	 ...
	 * Create the wizard.
	 */
	public SubKindPatternRefactoringPage(SubKindOccurrence skOccur) {
		super();
		this.skOccur = skOccur;

		setTitle(SubKindPattern.getPatternInfo().acronym + " Refactoring Options");
		setDescription("The follwing options can be used to refactor the model.");
	}

	public SubKindPatternWizard getAssCycWizard() {
		return (SubKindPatternWizard) getWizard();
	}
	...
	public void createControl(Composite parent) {
		...
		for (Association a : skOccur.getOnlyAssociations()) {
	...
	public IWizardPage getNextPage() {
		((SubKindPatternWizard) getWizard()).removeAllActions();

		if (btnDerive.getSelection() && assocCombo.getSelectionIndex() >= 0) {
			Association assoc = (Association) skOccur.getOnlyAssociations().get(assocCombo.getSelectionIndex());
			// Action =============================
			SubKindPatternAction newAction = new SubKindPatternAction(skOccur);
		...
		return ((SubKindPatternWizard) getWizard()).getFinishing();
	}
}
