				...
				for (Object element : (Collection<?>) o ) {
					...
					if (element instanceof Tuple){
						...
						if (queryReturnType1.isInstance(tuple.getValue(tupleField1))){
							...
							T field1Original = (T) PatternUtil.getOriginal((EObject)tuple.getValue(tupleField1), copier);
							
							Set<S> field2 = (Set<S>) tuple.getValue(tupleField2);
							ArrayList<S> field2Original = new ArrayList<>();
							for (S s : field2) {
								field2Original.add((S) PatternUtil.getOriginal((EObject)s, copier));
							}
			...
			if (o instanceof Collection<?>){
				...
				for (Object element : (Collection<?>) o ) {
					...
					if(queryReturnType.isInstance(element) && element instanceof EObject){
						...
						T original = (T) PatternUtil.getOriginal((EObject)element, copier);			
				...
				for (Object element : (Collection<?>) o ) {
					...
					if (element instanceof Tuple){
						...
						if(queryReturnType1.isInstance(tuple.getValue(tupleField1)) && queryReturnType2.isInstance(tuple.getValue(tupleField2)) 
							...
							T original1 = (T) PatternUtil.getOriginal((EObject)tuple.getValue(tupleField1), copier);
							S original2 = (S) PatternUtil.getOriginal((EObject)tuple.getValue(tupleField2), copier);
